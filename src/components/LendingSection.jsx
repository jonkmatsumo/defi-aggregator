...
const LendingSection = () => {
  const [lendingAssets, setLendingAssets] = useState({
    compound: [],
    aave: [],
    all: [],
  });
  ...
  const fetchLendingData = useCallback(async () => {
    if (!isConnected || !address) {
      setLendingAssets({
        ...
        all: [
          ...lendingService.current.getFallbackCompoundTokens(),
          ...lendingService.current.getFallbackAaveReserves(),
        ],
      });
      setUserBalances({
        compound: [],
  ...
  return (
    ...
    >
      ...
      {isConnected && (
        ...
        >
          <h4
            style={{ margin: "0 0 12px 0", fontSize: "14px", color: "#a0aec0" }}
          >
            Your Positions
          </h4>
      ...
      >
        {["supply", "withdraw", "borrow", "repay"].map(action => (
          <button
            ...
            style={{
              background:
                selectedAction === action ? getActionColor() : "#2d3748",
              border: "1px solid #4a5568",
              borderRadius: "8px",
              padding: "8px 16px",
              color: "white",
      ...
      <div style={{ marginBottom: "20px" }}>
        <div
          style={{ fontSize: "14px", color: "#a0aec0", marginBottom: "8px" }}
        >
          Select Platform
        </div>
        <div style={{ display: "flex", gap: "8px" }}>
          {LendingService.getPlatforms().map(platform => (
            <button
              ...
              style={{
                background:
                  selectedPlatform === platform.id ? "#667eea" : "#2d3748",
                border: "1px solid #4a5568",
                borderRadius: "8px",
                padding: "8px 16px",
                color: "white",
      ...
      <button
        ...
        style={{
          ...
          fontWeight: "bold",
          cursor:
            loading || !selectedToken || !amount || !isConnected
              ? "not-allowed"
              : "pointer",
          opacity:
            loading || !selectedToken || !amount || !isConnected ? 0.5 : 1,
        }}
      >
