...
const YourAssets = ({ maxAssets = 3, forceRefresh = false }) => {
  ...
  const fetchTokenBalances = useCallback(async () => {
    ...
    console.log(
      "YourAssets - Fetching real token balances for connected wallet"
    );
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const assetsWithValues = balances.map(asset => ({
        ...asset,
        value: TokenBalanceService.calculateUSDValue(
          asset.balance,
          asset.symbol
        ),
        balance: TokenBalanceService.formatBalance(
          asset.balance,
          asset.decimals
        ),
      }));
  ...
  const handleRefresh = () => {
    ...
    tokenBalanceService.current
      .fetchAllTokenBalances(publicClient, address, maxAssets)
      .then(balances => {
        if (!balances || balances.length === 0) {
          setAssets([]);
          return;
  ...
  const displayAssets = isConnected
    ? assets
    : TokenBalanceService.getFallbackAssets();
        ...
        {displayAssets.map((asset, index) => (
          <div
            ...
            style={{
              ...
              padding: "12px 0",
              borderBottom:
                index < displayAssets.length - 1 ? "1px solid #4a5568" : "none",
            }}
          >
