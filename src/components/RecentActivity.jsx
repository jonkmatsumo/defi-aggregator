...
const RecentActivity = ({ transactionCount = 3, forceRefresh = false }) => {
  ...
  const getTransactionHistory = useCallback(
    async (client, userAddress, count) => {
      try {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    console.log(
      "RecentActivity - Fetching transactions for address:",
      address?.slice(0, 10) + "..."
    );
    setLoading(true);
    setError(null);
  ...
  const displayTransactions = isConnected
    ? transactions
