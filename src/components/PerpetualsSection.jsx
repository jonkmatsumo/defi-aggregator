        ...
        switch (data.type) {
          ...
          case "connection":
            ...
            if (data.status === "disconnected") {
              setChartError(
                "Price feed disconnected. Attempting to reconnect..."
              );
            } else {
              setChartError("");
            }
    ...
    if (isConnected && walletClient && publicClient) {
      ...
      const initializeGMX = async () => {
        try {
          ...
          if (!gmxAddress) {
            setError(
              "GMX not supported on this network. Please switch to Arbitrum or Avalanche."
            );
            return;
          }
          ...
          const contract = new ContractConstructor(
            gmxAddress,
            GMX_ABI,
            walletClient
          );
  ...
  const handleError = error => {
    ...
    switch (true) {
      case error.message.includes("Insufficient funds") ||
        ...
        errorMessage =
          "Insufficient balance to open the position. Please check your wallet balance.";
        break;
      case error.message.includes("Invalid leverage") ||
        error.message.includes("leverage"):
        errorMessage =
          "Invalid leverage value. Please select a leverage between 1x and 100x.";
        break;
      case error.message.includes("Margin call") ||
        error.message.includes("liquidation"):
        errorMessage =
          "Position is close to liquidation. Please add more margin or close the position.";
        break;
      case error.message.includes("Slippage") ||
        error.message.includes("price impact"):
        errorMessage =
          "Price impact too high. Try reducing position size or increasing slippage tolerance.";
        break;
      ...
      default:
        errorMessage =
          error.message || "Failed to open position. Please try again.";
        break;
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span
            style={{ fontSize: "clamp(14px, 1.5vw, 18px)", fontWeight: "600" }}
          >
            {marketData.symbol}
          </span>
          <span
      ...
      {process.env.NODE_ENV !== "test" && (
        <div
          style={{
            ...
            marginBottom: "clamp(12px, 1.5vw, 20px)",
            padding: "clamp(8px, 1vw, 12px)",
            backgroundColor: isChartConnected
              ? "rgba(72, 187, 120, 0.1)"
      ...
      >
        ...
        >
          ...
          >
            ...
            >
                    ...
                    chartData
                      .map((point, i) => {
                        ...
                        const normalizedPrice =
                          (point.price - chartBounds.minPrice) /
                          chartBounds.priceRange;
              ...
              <g fill="#a0aec0" fontSize="12" textAnchor="end">
                {[0, 1, 2, 3, 4].map(i => {
                  const price =
                    chartBounds.maxPrice - (i * chartBounds.priceRange) / 4;
                  const y = i * 100 + 15;
                  return (
          ...
          >
            {[
              ...
              {
                label: "24h Volume",
                value: `${formatNumber(marketData.volume24h)} BTC`,
              },
        ...
        >
          ...
          {[
            {
              ...
              placeholder: `0.00 ${tokenPair.split("/")[0]}`,
            },
            {
              label: "Stop Loss",
              value: stopLoss,
          ...
          <button
            ...
            disabled={
              isLoading || !isConnected || !positionSize || !gmxContract
            }
            style={{
              width: "100%",
          ...
          >
            {isLoading
              ? "Processing..."
              : `Open ${positionType.charAt(0).toUpperCase() + positionType.slice(1)} Position`}
          </button>
        </div>
      </div>
          ...
          >
            ...
            {openPositions.map((position, index) => {
              ...
              return (
                ...
                >
                  <div style={{ fontSize: "clamp(8px, 1vw, 12px)" }}>
                    {position.id}
                  </div>
                  <div>{position.symbol}</div>
                  <div
                    style={{
                      color: position.side === "Long" ? "#48bb78" : "#f56565",
                    }}
                  >
                    {position.side}
                  </div>
                  <div>
                    {position.size} {position.symbol?.split("/")[0] || "BTC"}
                  </div>
                  ...
                  <div>
                    {formatCurrency(details.marginUsed || position.size)}
                  </div>
                  <div>
                    <button
