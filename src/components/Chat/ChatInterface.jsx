...
const ChatInterface = () => {
  ...
  const serverUrl = process.env.REACT_APP_GENAI_SERVER_URL || 'ws://localhost:3001';
  ...
  const [agentService] = useState(() => {
    try {
      ...
      console.warn('Failed to initialize AgentServiceClient, using mock service:', error);
      return new MockAgentService();
    }
  });

  // Fallback service for when server is unavailable
  const [fallbackService] = useState(() => new MockAgentService({ minDelay: 0, maxDelay: 100 }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        if (agentService.isAgentServiceClient || agentService.constructor.name === 'AgentServiceClient') {
    ...
    <div className="chat-interface">
      ...
      {agentService instanceof AgentServiceClient && (
        <div className={`connection-status ${connectionStatus}`}>
          ...
          {connectionStatus === 'error' && '🔴 Connection Error - Using Offline Mode'}
        </div>
      )}
