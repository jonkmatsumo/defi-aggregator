...
class TokenBalanceService {
  ...
  async fetchTokenMetadata(client, tokenAddress) {
    ...
    const networkMap = { 1: 'ethereum', 137: 'polygon', 56: 'bsc', 42161: 'arbitrum', 10: 'optimism' };
  ...
  async fetchNativeBalance(client, address) {
    ...
    try {
      ...
      const nativeToken = result.tokens?.find(t => 
      ...
      const balanceData = {
        ...
        value: nativeToken.balanceUSD || TokenBalanceService.calculateUSDValue(nativeToken.balance, nativeToken.symbol),
  ...
  getNativeTokenName(chainId) {
    const names = { 1: 'Ether', 137: 'MATIC', 56: 'BNB', 42161: 'Ether', 10: 'Ether' };
  ...
  getNativeTokenColor(chainId) {
    const colors = { 1: '#627eea', 137: '#8247e5', 56: '#f3ba2f', 42161: '#ff6b35', 10: '#ff0420' };
  ...
  async fetchTokenBalance(client, tokenAddress, userAddress) {
    ...
    try {
      ...
      const balanceData = {
        ...
        value: result.balanceUSD || TokenBalanceService.calculateUSDValue(result.balance, result.symbol),
        ...
        for (const token of result.tokens) {
          if (parseFloat(token.balance) > 0) {
            balances.push({
              ...
              value: token.balanceUSD || TokenBalanceService.calculateUSDValue(token.balance, token.symbol),
