  ...
  describe("Property-Based Tests", () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (message, history) => {
            const response = await mockAgentService.sendMessage(
              message,
              history
            );
    ...
    it("should return appropriate UI intent for messages matching predefined patterns", async () => {
      ...
      const patternMappings = [
        ...
        {
          keywords: ["lend", "lending", "apy", "earn"],
          component: "LendingSection",
        },
        {
          keywords: ["balance", "asset", "portfolio"],
          component: "YourAssets",
        },
        {
          keywords: ["perpetual", "perp", "leverage"],
          component: "PerpetualsSection",
        },
        {
          keywords: ["activity", "history", "transaction"],
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (patternIndex, prefix, suffix, caseVariation) => {
            ...
            const keyword =
              pattern.keywords[
                Math.floor(Math.random() * pattern.keywords.length)
              ];
  ...
  describe("Unit Tests", () => {
      ...
      it("should return PerpetualsSection component for perpetual-related messages", async () => {
        ...
        for (const message of testMessages) {
          ...
          expect(response.content).toBe(
            "You can open leveraged positions here:"
          );
        }
      });
    ...
    describe("Default Response", () => {
      it("should return default response for unrecognized input", async () => {
        ...
        for (const message of unrecognizedMessages) {
          const response = await mockAgentService.sendMessage(message, []);
          expect(response.content).toBe(
            "I can help you with swaps, checking gas prices, viewing your assets, and more. What would you like to do?"
          );
    ...
    describe("Response Delay", () => {
      it("should have response delay within expected range (200-400ms)", async () => {
        ...
        const serviceWithDelay = new MockAgentService({
          minDelay: 200,
          maxDelay: 400,
        });
