...
describe('TokenBalanceService', () => {
  ...
  describe('fetchNativeBalance', () => {
    ...
    it('should return null when address is missing', async () => {
      ...
      const result = await tokenBalanceService.fetchNativeBalance(mockClient, null);
      expect(result).toBeNull();
    });
  ...
  describe('fetchTokenBalance', () => {
    ...
    it('should return null when addresses are missing', async () => {
      ...
      const result = await tokenBalanceService.fetchTokenBalance(mockClient, null, null);
      expect(result).toBeNull();
    });
    ...
    it('should fetch token balance from backend API', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith(`/api/balances/${userAddress}`, {
        network: 'ethereum',
        tokenAddress,
        includeUSDValues: 'true'
      });
  ...
  describe('fetchAllTokenBalances', () => {
    it('should return empty array when client is missing', async () => {
      ...
      const result = await tokenBalanceService.fetchAllTokenBalances(null, userAddress);
      expect(result).toEqual([]);
    });
    ...
    it('should return empty array when address is missing', async () => {
      ...
      const result = await tokenBalanceService.fetchAllTokenBalances(mockClient, null);
      expect(result).toEqual([]);
    });
    ...
    it('should fetch all token balances from backend API', async () => {
      ...
      const result = await tokenBalanceService.fetchAllTokenBalances(mockClient, userAddress, 3);
    ...
    it('should respect maxAssets parameter', async () => {
      ...
      const mockResponse = {
        tokens: [
          { symbol: 'ETH', name: 'Ether', balance: '2.0', balanceUSD: '$4,000', decimals: 18 },
          { symbol: 'USDC', name: 'USD Coin', balance: '1000', balanceUSD: '$1,000', decimals: 6 },
          { symbol: 'DAI', name: 'Dai', balance: '500', balanceUSD: '$500', decimals: 18 }
        ]
      };
    ...
    it('should filter out zero balances', async () => {
      ...
      const mockResponse = {
        tokens: [
          { symbol: 'ETH', name: 'Ether', balance: '2.0', balanceUSD: '$4,000', decimals: 18 },
          { symbol: 'USDC', name: 'USD Coin', balance: '0', balanceUSD: '$0', decimals: 6 }
        ]
      };
  ...
  describe('fetchPortfolioValue', () => {
    ...
    it('should fetch portfolio value from backend API', async () => {
      ...
      const result = await tokenBalanceService.fetchPortfolioValue(userAddress, ['ethereum', 'polygon']);

      expect(apiClient.get).toHaveBeenCalledWith(`/api/portfolio/${userAddress}`, {
  ...
  describe('static methods', () => {
    ...
    describe('formatBalance', () => {
      ...
      it('should format very small balances', () => {
        expect(TokenBalanceService.formatBalance('0.00001', 18)).toBe('< 0.0001');
      });
      ...
      it('should format large balances with commas', () => {
        expect(TokenBalanceService.formatBalance('1000000', 18)).toBe('1,000,000');
      });
    });
