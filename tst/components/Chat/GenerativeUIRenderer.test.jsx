...
describe("GenerativeUIRenderer", () => {
  ...
  describe("Unit Tests", () => {
    ...
    it("should show error message when component name is not found in registry", () => {
      ...
      expect(
        screen.getByText(
          /Unable to render component: NonExistentComponent not found/i
        )
      ).toBeInTheDocument();
    });
    ...
    it("should show error when uiIntent is null", () => {
      ...
      expect(
        screen.getByText(/Invalid UI intent: Expected an object/i)
      ).toBeInTheDocument();
    });
    ...
    it("should show error when component name is missing", () => {
      ...
      expect(
        screen.getByText(
          /Invalid UI intent: Missing or invalid component name/i
        )
      ).toBeInTheDocument();
    });
  ...
  describe("Property-Based Tests", () => {
          ...
          fc.property(
            ...
            (componentName, props) => {
              ...
              try {
                ...
                expect(screen.getByTestId("prop-testId")).toHaveTextContent(
                  props.testId
                );
                expect(screen.getByTestId("prop-count")).toHaveTextContent(
                  String(props.count)
                );
                expect(screen.getByTestId("prop-enabled")).toHaveTextContent(
    ...
    describe("Property 12: Unknown component handling", () => {
      it("should display error messages for invalid component names without crashing", () => {
        ...
        const invalidComponentNameArb = fc
          .string({ minLength: 1, maxLength: 50 })
          .filter(
            name => !validComponentNames.has(name) && name.trim().length > 0
          );
        ...
        fc.assert(
          fc.property(invalidUIIntentArb, uiIntent => {
            ...
            const errorElements = screen.queryAllByText(
              /Invalid UI intent|Unable to render component/i
            );
            expect(errorElements.length).toBeGreaterThan(0);
