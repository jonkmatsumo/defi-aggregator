...
describe("LendingSection", () => {
  ...
  it("shows demo mode when not connected", async () => {
    ...
    expect(
      screen.getByText("Connect your wallet to start lending and borrowing")
    ).toBeInTheDocument();
  });
  ...
  it("handles transaction failure", async () => {
    ...
    mockServiceInstance.supplyTokens.mockRejectedValue(
  ...
  it("handles error when fetching lending data fails", async () => {
    const mockServiceInstance = mockLendingService();
    mockServiceInstance.fetchAllLendingAssets.mockRejectedValue(
      new Error("API Error")
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Failed to fetch lending data")
      ).toBeInTheDocument();
    });
  });
