...
export class EducationalGenerator {
  generatePriceContext(result) {
    ...
    if (result.change_24h !== undefined) {
      tips.push(
        '24h change indicates short-term momentum; large moves can imply higher volatility.'
      );
    }
    if (result.volume_24h) {
      tips.push(
        'High 24h volume often signals stronger liquidity and tighter spreads.'
      );
    }
    if (result.market_cap) {
      tips.push(
        'Market cap helps compare asset size; larger caps typically move more slowly.'
      );
    }
  ...
  generateGasExplanation(result) {
    ...
    const tips = [];
    tips.push(
      'Gas prices reflect network demand; higher gwei means faster inclusion but higher cost.'
    );
    tips.push('Consider submitting during off-peak hours to reduce fees.');
    if (result.transaction_type || result.transactionType) {
      tips.push(
        'Different transaction types (swap vs transfer) consume different gas amounts.'
      );
    }
    return tips;
  }
  ...
  generateLendingContext(result) {
    ...
    const tips = [];
    tips.push(
      'Supply APY is what you earn for depositing; Borrow APY is what you pay when borrowing.'
    );
    tips.push(
      'Utilization near 100% can increase variable borrow rates; monitor frequently.'
    );
    tips.push(
      'Diversify protocols to reduce smart contract and liquidity risk.'
    );
    return tips;
  }
  ...
  generateProtocolExplanation(protocol) {
    ...
    switch (key) {
      case 'aave':
        return [
          'Aave is a major lending protocol supporting variable and stable borrow rates.',
        ];
      case 'compound':
        return [
          'Compound is a lending protocol where rates adjust algorithmically with utilization.',
        ];
      case 'maker':
        return [
          'Maker issues the DAI stablecoin; vault users generate DAI against collateral.',
        ];
      default:
        return [
          'Research protocol security, audits, and liquidity before depositing significant funds.',
        ];
    }
  }
  ...
  generateRecommendations(toolResult) {
    ...
    if (toolResult.toolName === 'get_gas_prices') {
      tips.push(
        'If not time-sensitive, choose slower gas tiers to save on fees.'
      );
    }
    if (
      toolResult.toolName === 'get_crypto_price' &&
      toolResult.result?.change_24h < 0
    ) {
      tips.push(
        'Price is down 24h; consider whether this is a dip or part of a larger trend.'
      );
    }
    if (toolResult.toolName === 'get_lending_rates') {
      tips.push(
        'Compare borrow APY vs supply APY to avoid negative carry when leveraging.'
      );
    }
    return tips.length ? tips : null;
  }
    ...
    for (const tr of toolResults) {
      ...
      switch (tr.toolName) {
        ...
        case 'get_token_balance': {
          tips.push(
            'Review token allocations and consider diversification across assets and networks.'
          );
          break;
        }
        default:
