...
describe('SystemPromptManager', () => {
      ...
      await fc.assert(
        fc.property(
          ...
          config => {
            ...
            const validation =
              manager.validatePromptCompleteness('defi_assistant');
  ...
  describe('Property 2: System prompt tool instruction inclusion', () => {
      ...
      await fc.assert(
        fc.property(
          ...
          toolDefinitions => {
            ...
            const validation =
              manager.validateToolInstructions('defi_assistant');
            expect(validation.valid).toBe(true);
            expect(validation.missing).toHaveLength(0);
            expect(validation.hasToolGuidelines).toBe(true);

            // Format prompt with tools
            const promptWithTools = manager.formatPromptWithTools(
              toolDefinitions,
              'defi_assistant'
            );
    ...
    test('should handle empty tool definitions gracefully', async () => {
      await fc.assert(
        fc.property(fc.constantFrom([], null, undefined), emptyTools => {
          const manager = new SystemPromptManager();

          // Should still provide valid prompt without tools
          const prompt = manager.formatPromptWithTools(
            emptyTools,
            'defi_assistant'
          );
          expect(prompt).toBeDefined();
      ...
      await fc.assert(
        fc.property(
          ...
          toolDefinitions => {
            ...
            const toolsWithParams = toolDefinitions.filter(
              tool =>
                tool.parameters &&
            ...
            toolsWithParams.forEach(tool => {
              ...
              paramEntries.forEach(([paramName, paramDef]) => {
                ...
                const escapedParamName = cleanParamName.replace(
                  /[.*+?^${}()|[\]\\]/g,
                  '\\$&'
                );

                const requiredPattern = new RegExp(
                  `${escapedParamName}.*required`,
                  'i'
                );
                const optionalPattern = new RegExp(
                  `${escapedParamName}.*optional`,
                  'i'
                );
                ...
                expect(
                  isRequired ? hasRequiredMatch : hasOptionalMatch
                ).toBeTruthy();
  ...
  describe('Property 3: System prompt expertise specification', () => {
      ...
      await fc.assert(
        fc.property(
          ...
          config => {
            ...
            const validation =
              manager.validateExpertiseSpecification('defi_assistant');
            expect(validation.valid).toBe(true);
            expect(validation.missing).toHaveLength(0);
      ...
      await fc.assert(
        fc.property(
          ...
          promptUpdate => {
            ...
            expect(metadata.identity.expertise).toEqual(
              promptUpdate.identity.expertise
            );
            expect(metadata.identity.personality).toBe(
              promptUpdate.identity.personality
            );
    ...
    test('should provide comprehensive expertise coverage for DeFi operations', async () => {
      await fc.assert(
        fc.property(fc.constant('defi_assistant'), context => {
          const manager = new SystemPromptManager();
  ...
  describe('Integration Tests', () => {
    test('should integrate all properties for complete system prompt functionality', async () => {
      ...
      const fullPrompt = manager.formatPromptWithTools(
        mockTools,
        'defi_assistant'
      );
