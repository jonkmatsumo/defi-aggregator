...
describe('ComponentIntentGenerator', () => {
      ...
      fc.assert(
        fc.property(
          ...
          testData => {
            ...
            const result = generator.generateIntent(
              toolResults,
              userMessage,
              llmResponse
            );
    ...
    test('should support multiple component intents in a single response', () => {
      fc.assert(
        fc.property(
          ...
          fc
            .shuffledSubarray(
              ['get_gas_prices', 'get_token_balance', 'get_lending_rates'],
          ...
          (toolResults, userMessage, llmResponse) => {
            ...
            const uniqueToolNames = [
              ...new Set(toolResults.map(tr => tr.toolName)),
            ];

            const isValidMultipleIntents =
              result === null ||
              (Array.isArray(result) &&
                result.length >= 1 &&
            ...
            const hasUniqueComponents =
              result === null ||
              (Array.isArray(result) &&
                new Set(result.map(intent => intent.component)).size ===
                  result.length);
  ...
  describe('Unit Tests', () => {
    ...
    test('should handle null/undefined inputs gracefully', () => {
      ...
      expect(
        generator.generateIntent(undefined, undefined, undefined)
      ).toBeNull();
