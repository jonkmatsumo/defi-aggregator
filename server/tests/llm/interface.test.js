...
class TestLLM extends LLMInterface {
  ...
  async generateStreamingResponse(messages, tools = [], onChunk, _options = {}) {
...
describe('LLM Interface', () => {
  describe('LLMInterface Base Class', () => {
    test('should throw error for unimplemented generateResponse', async () => {
      ...
      await expect(llm.generateResponse([])).rejects.toThrow('generateResponse must be implemented by subclass');
    });

    test('should throw error for unimplemented generateStreamingResponse', async () => {
      const llm = new LLMInterface({ provider: 'test' });
      const mockOnChunk = jest.fn();
      await expect(llm.generateStreamingResponse([], [], mockOnChunk)).rejects.toThrow('generateStreamingResponse must be implemented by subclass');
    });
  });
    ...
    test('Property 10: LLM response processing', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (responseContent, toolCalls) => {
          ...
          const result = await llm.generateResponse([{ role: 'user', content: 'test' }]);
    ...
    test('Property 33: Streaming response forwarding', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (_contentChunks) => {
          ...
          await llm.generateStreamingResponse([{ role: 'user', content: 'test' }], [], onChunk);

          // Should forward content chunks
          const contentChunksReceived = receivedChunks.filter(chunk => chunk.type === 'content');
    ...
    test('Property 34: Streaming completion signaling', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (_fullContent) => {
          ...
          const completionChunks = receivedChunks.filter(chunk => chunk.type === 'done' && chunk.done === true);
    ...
    test('Property 35: Streaming error handling', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (errorMessage) => {
          ...
          await expect(llm.generateStreamingResponse([{ role: 'user', content: 'test' }], [], onChunk))
            .rejects.toThrow();

          // Should send error chunk to client
          const errorChunks = receivedChunks.filter(chunk => chunk.type === 'error');
    ...
    test('Property 36: Streaming backward compatibility', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (content) => {
          ...
          const streamResult = await llm.generateStreamingResponse([{ role: 'user', content: 'test' }], [], onChunk);
